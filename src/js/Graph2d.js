...
var Graph2d = /** @class */ (function () {
     ...
     */
    Graph2d.fillPlayerList = function () {
     ...
     */
    Graph2d.fillItemList = function (json, method) {
        var group;
        var newItem;
        for (var i in json) {
            var stats = json[i];
            ...
            var time = this.convertMillisToTime(ts);
            var time_formatted = this.convertTime(time)[3];
            var player = stats[0]["player"];
     ...
     */
    Graph2d.visualizeGraph2d = function (method) {
     ...
     */
    Graph2d.updateGraph2dOptions = function (method) {
        if (method == "/analyzePressing2d") {
            Graph2d.options = {
                // clickToUse: true,
                stack: false,
            ...
            var colors = ['#e5524a', '#187fce', '#008d68', '#ffa800', '#7b457b', '#e5524a', '#187fce', '#008d68', '#ffa800', '#7b457b', '#e5524a'];
            Graph2d.groups = new vis.DataSet();
            // create groups dynamically
            for (var i in this.players) {
                var id = this.players[i].match(/\[(.*?)\]/)[1].split(":")[1];
                var name_1 = this.players[i].split("[")[0];
                Graph2d.groups.add({
                    id: id,
                    content: name_1,
                    className: 'graph2d',
     ...
     */
    Graph2d.convertMillisToTime = function (ts) {
     ...
     */
    Graph2d.convertTime = function (time) {
        ...
        var formatted = "2020-1-1 " + hours + ":" + mins + ":" + secs;
     ...
     */
    Graph2d.clearItemList = function () {
        Graph2d.itemList = [];
        Graph2d.players = [];
        // remove old content in graph2d visualization
        var div = document.getElementById('visualization2d');
