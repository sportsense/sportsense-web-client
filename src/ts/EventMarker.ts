...
class EventMarker {
    ...
    public time: any;
    private parent_chain: EventChain;
    private static markers: EventMarker[] = [];

    /**
     * this constructor adds an event marker on the canvas at a given position
     */
    constructor(location: any[], id: string, time: any, videotime: any, ec: EventChain) {

        let strokeWidth = CONFIG.MARKER_SIZE;
        let radius= CONFIG.MARKER_SIZE / 2;
        ...
        for (let i = 0; i < EventMarker.markers.length; i++){
     ...
     * @param marker
     */
    public static push(marker: EventMarker): void {
     ...
     * @param highlight
     */
    public static changeMarkerColor(markers: EventMarker[], highlight: number): void {
    ...
    public static clearMarkers(): void {
        for (let i in EventMarker.markers) {
     ...
     * @param id
     */
    public static deleteMarker(id: string): void {
        var newmarkers = [];
        for (let em of EventMarker.markers) {
            if (id != em.markerid) {
                newmarkers.push(em);
            } else {
                em.removeMarker();
     ...
     * @returns {EventMarker[]}
     */
    public static getMarkerList(): EventMarker[] {
     ...
     * @returns {any}
     */
    public static getMarker(id: string){
        for(let marker of EventMarker.markers){
     ...
     * @param ec
     */
    public setNewParent(ec: EventChain){
        this.parent_chain = ec;
    }
