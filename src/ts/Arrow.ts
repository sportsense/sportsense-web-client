...
class Arrow {
    ...
    private static arrows: any[] = [];
    public static active_arrow: Arrow = null;

    constructor(points: number[], eventchain: EventChain) {
        this.eventchain = eventchain;
        let p1 = DrawingArea.dbToCanvasCoordinates(points[0][0], points[0][1]);
        let p2 = DrawingArea.dbToCanvasCoordinates(points[1][0], points[1][1]);
        ...
        this.arrowhead = new fabric.Triangle({
            left: coords[2],            // triangle is positioned on p2
            top: coords[3],
            strokeWidth: 1,
            width:8,height:8,
            stroke: false,
    ...
    public static addArrow(arrow: Arrow): void {
    ...
    public changeColor(highlight: number): void {
        if (highlight == 1) {
    ...
    public static deleteAllArrows(): void {
