...
class Timeline {
    ...
    private static MIN = new Date(2000, 0, 1, 0, 0, 0, 0);
    private static MAX = new Date(2000, 0, 1, 1, 30, 0, 0);
    ...
    public static addHighlight(startTs, endTs, text, groupId): void{
        let start = Timeline.convertTime(startTs);
        let end = Timeline.convertTime(endTs);
        let newItem = {id: "highlight"+Timeline.highlightCounter, group: groupId, start: start[2], end: end[2], content: text, editable: false};
    ...
    private static convertTime(time: string): any[]{
        ...
        let mins = parseInt(times[0]);
        let secs = parseInt(times[1]);
        ...
        let formated = "2000-1-1 " + hours + ":" + mins + ":" + secs;
    ...
    public static getTimeFilter(): string{
        if(Timeline.filterActive){
            let t = new Date(2000,0,1,0,0,0,0).getTime();
            let t1 = Timeline.timeline.getCustomTime("slider1").getTime()-t;
            let t2 = Timeline.timeline.getCustomTime("slider2").getTime()-t;
            if(t1 < t2){
